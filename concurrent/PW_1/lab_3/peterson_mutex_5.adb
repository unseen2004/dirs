...
-- Processes 
  Nr_Of_Processes : constant Integer := 2;  -- Peterson's algorithm is for 2 processes only
...
-- Peterson's Algorithm Variables
  Flag : array (0 .. 1) of Boolean := (False, False);  -- Process wants to enter critical section
  Turn : Integer := 0;  -- Whose turn it is when both processes want to enter (0 or 1)
...
-- Types, procedures and functions
  ...
  begin
    ...
    Put_Line("PETERSON_ALGORITHM;"); -- Indicate this is Peterson's algorithm
  end Printer;
  ...
  task body Process_Task_Type is
    ...
    Other_Process : Integer;  -- ID of the other process
  ...
  begin
    accept Init(Id: Integer; Seed: Integer; Symbol: Character) do
      ...
      Other_Process := 1 - Id;  -- If Id=0 then Other=1, if Id=1 then Other=0
    ...
    for Step in 0 .. Nr_of_Steps/4 - 1  loop  -- TEST !!!
      ...
      Flag(Process.Id) := True;
      
      -- Step 2: Give turn to the other process (be polite)
      Turn := Other_Process;
      
      -- Step 3: Wait while the other process wants to enter AND it's their turn
      while Flag(Other_Process) and then Turn = Other_Process loop
      ...
      -- Simply indicate we no longer want to enter critical section
