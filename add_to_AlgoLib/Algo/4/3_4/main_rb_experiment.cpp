...
int main() {
    ...
    std::ofstream ofs_inc("results_increasing_rb.csv");
    std::ofstream ofs_rand("results_random_rb.csv");
    ...
    for (int n : ns) {
        ...
        // Case A: insert 1..n in increasing order, then delete in random order
        ...
        uint64_t ops = static_cast<uint64_t>(n) * 2 * trials;
        ofs_inc << n << ","
        ...
        // Case B: insert random permutation, delete random
    ...
    ofs_inc.close();
    ofs_rand.close();
    std::cout << "RB Experiment data written to results_increasing_rb.csv and results_random_rb.csv" << std::endl;
    return 0;
}
