...
-- Processes 
  Nr_Of_Processes : constant Integer := 2;  -- Dekker's algorithm is for 2 processes only
...
-- Dekker's Algorithm Variables
  Want : array (0 .. 1) of Boolean := (False, False);  -- Process wants to enter critical section
  Turn : Integer := 0;  -- Whose turn it is (0 or 1)
...
-- Types, procedures and functions
  ...
  begin
    ...
    Put_Line("DEKKER_ALGORITHM;"); -- Indicate this is Dekker's algorithm
  end Printer;
  ...
  begin
    ...
    for Step in 0 .. Nr_of_Steps/4 - 1  loop  -- TEST !!!
      ...
      Want(Process.Id) := True;
      
      -- Step 2: Wait while the other process wants to enter AND it's their turn
      while Want(Other_Process) loop
        if Turn = Other_Process then
          -- It's the other process's turn, so we back off
          Want(Process.Id) := False;
          
          -- Wait until it's our turn
          while Turn = Other_Process loop
